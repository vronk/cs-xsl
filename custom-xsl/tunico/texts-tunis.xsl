...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml"
    xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:hits="http://clarin.eu/fcs/dataview/hits"
    ...
    exclude-result-prefixes="xsl exsl xd tei fcs sru hits">
    <xsl:import href="../../fcs/result2view_v1.xsl"/>
    <xsl:import href="../../commons_v2.xsl"/>
    <xsl:output method="xhtml" media-type="text/xhtml" indent="yes" omit-xml-declaration="yes"
    ...
    <xsl:template name="callback-header">
        ...
        <style type="text/css">
            body{
                font: 13px/1.5 AndikaW, 'Andika', serif;
            }</style>
        ...
        <xsl:if test="@xml:lang = 'eng'">
            ...
            <div class="nyms">
                <div class="official nym-wrapper">
                    ...
                    <xsl:apply-templates select="../tei:name[@xml:lang = 'ara-x-DMG']"
                        mode="record-data"/>
                </div>
                    ...
                    <div class="local nym-wrapper">
                        ...
                        <xsl:choose>
                            <xsl:when test="../tei:name[@type = 'araLoc']">
                                <xsl:apply-templates select="../tei:name[@type = 'araLoc']"
                                    mode="record-data"/>
                            </xsl:when>
                            <xsl:otherwise>
                        ...
                        <xsl:choose>
                            <xsl:when test="../tei:name[@type = 'latLoc']">
                                <xsl:apply-templates select="../tei:name[@type = 'latLoc']"
                                    mode="record-data"/>
                            </xsl:when>
                            <xsl:otherwise>
    ...
    <xd:doc>
        <xd:desc>Transforms one fcs:Resource <xd:p>This supersedes the generic template because we
                want a fixed order in which the various data views are transformed (metadata
                last)</xd:p>
        </xd:desc>
    </xd:doc>
    <xsl:template match="fcs:Resource" mode="record-data">
        <xsl:apply-templates select="./fcs:ResourceFragment/fcs:DataView[not(@type = 'metadata')]"
            mode="record-data">
            <xsl:with-param name="resource-pid" select="@pid"/>
        </xsl:apply-templates>
        <xsl:apply-templates select="./fcs:ResourceFragment/fcs:DataView[@type = 'metadata']"
            mode="record-data"/>
    </xsl:template>
    <xsl:template match="fcs:DataView[not(@ref) and @type = 'facs']" mode="record-data">
        <xsl:param name="resource-pid"/>
        <div class="data-view facs xsl-outer">
    ...
    <xsl:template match="tei:w | tei:pc" mode="record-data">
        <xsl:variable name="classes">
            <xsl:choose>
                <xsl:when test="@type = preceding-sibling::*[1]/@type">
                    <xsl:value-of
                        select="concat('tei-', local-name(), ' tei-type-', @type, ' lang-fr')"/>
                </xsl:when>
                <xsl:when test="@type">
                    <xsl:value-of
                        select="concat('tei-', local-name(), ' tei-type-', @type, ' lang-fr xsl-first-of-group')"
                    />
                </xsl:when>
                <xsl:when test="preceding-sibling::tei:w[1]/@type"> tei-w lang-aeb
                    xsl-first-of-group </xsl:when>
        ...
        <xsl:choose>
                        ...
                        <xsl:with-param name="x-context">
                            <xsl:choose>
                                <xsl:when test="../../tei:ref/@target | ../tei:ref/@target">
                                    <xsl:value-of
                                        select="../../tei:ref/@target | ../tei:ref/@target"/>
                                </xsl:when>
                                <xsl:otherwise>
            ...
            <xsl:otherwise>
                <span class="{$classes}">
                    <xsl:if test="not(exists(@rend)) or @rend != 'glue'">
                        <xsl:text> </xsl:text>
                    </xsl:if>
                    <xsl:apply-templates mode="record-data"/>
                </span>
    ...
    <xd:doc>
        <xd:desc>Return the result if there is exactly one result <xd:p>Maybe this should be
                standard?</xd:p>
        </xd:desc>
    </xd:doc>
    <xsl:template
        match="sru:records[(count(sru:record) = 1) and (.//fcs:DataView[@type = 'application/x-clarin-fcs-kwic+xml'] or .//tei:entry)]"
        mode="table">
    ...
    <xsl:template match="hits:Hit" mode="result-data-table">
        ...
        <td class="left context">
            <xsl:apply-templates
                select="subsequence(preceding-sibling::*, count(preceding-sibling::*) - $num-context-items, $num-context-items)"
                mode="record-data"/>
        </td>
        <td class="kw hilight">
            <xsl:apply-templates mode="record-data"/>
        ...
        <td class="right context">
            <xsl:apply-templates select="subsequence(following-sibling::*, 1, $num-context-items)"
                mode="record-data"/>
        </td>
    </xsl:template>
</xsl:stylesheet>