...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml"
    ...
    xmlns:exist="http://exist.sourceforge.net/NS/exist" xmlns:exsl="http://exslt.org/common"
    ...
    <xd:doc scope="component">
        <xd:desc>
            <xd:p>This template generates the CSS classes from any element following the
                convention:</xd:p>
            <xd:ul>
            ...
            <xd:p>The various parts of a name are concatenated with hyphens. Blanks in @rend are
                replaced by underscores.</xd:p>
        </xd:desc>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>revisionDesc is not meaningful right now, content may be referenced
            elsewhere</xd:desc>
    </xd:doc>
    <xsl:template match="tei:revisionDesc" mode="record-data"/>
    <xsl:template match="tei:change">
        <xsl:value-of select="text()"/>
        <xsl:if test="tei:fs"> Last changed <xsl:value-of
                select="tei:fs/tei:f[@name = 'when']/tei:symbol/@value"/> by <xsl:value-of
                select="tei:fs/tei:f[@name = 'who']/tei:symbol/@value"/>
        </xsl:if>
    </xsl:template>
    <xd:doc>
        <xd:desc>encodingDesc is not meaningful right now, content may be referenced
            elsewhere</xd:desc>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>Customization point that returns a list of all mappings of characters that are used
            in this resource. <xd:p>Note: There is no meaningful default.</xd:p>
            <xd:p>
                Example:<xd:pre>
        ...
        <tr>
            ...
            <xsl:for-each select="exsl:node-set($mapping-list)/tei:mapping">
                ...
                <td class="tei-mapping-{$type}"><xsl:call-template name="string-join">
                        <xsl:with-param name="nodes-to-join"
                            select="exsl:node-set($cur-char)/tei:mapping[@type = $type]"/>
                        <xsl:with-param name="join-with">,</xsl:with-param>
                    </xsl:call-template></td>
            </xsl:for-each>
            <td class="tei-charProp-localName"><xsl:value-of select="tei:charProp/tei:localName"
                /></td>
            <td class="tei-charProp-unicodeName"><xsl:value-of select="tei:charProp/tei:unicodeName"
                /></td>
        </tr>
    </xsl:template>
    <xd:doc>
    ...
    <xsl:template match="tei:monogr" mode="record-data">
        <span class="tei-authors"><xsl:apply-templates mode="record-data" select="tei:author"
            /></span><span class="xsl-separator tei-title tei-author">: </span><span
            class="tei-titles"><xsl:apply-templates mode="record-data" select="tei:title"
    ...
    <xsl:template match="tei:analytic" mode="record-data">
        <span class="tei-authors"><xsl:apply-templates mode="record-data" select="tei:author"
            /></span><span class="xsl-separator tei-title-tei-author-sep">: </span><span
            class="tei-titles"><xsl:apply-templates mode="record-data" select="tei:title"/></span>
        in </xsl:template>
    <xd:doc>
    ...
    <xsl:template match="tei:series" mode="record-data">
        <xsl:if test="./text() != ''">
            <div class="tei-series">
                <xsl:apply-templates mode="record-data"/><span class="xsl-separator tei-imprint-sep"
                    >.</span></div>
        </xsl:if>
    </xsl:template>
    <xd:doc>
    ...
    <xsl:template match="tei:series[parent::tei:imprint]" mode="record-data">
        <xsl:if test="./text() != ''">
            <span class="xsl-separator tei-imprint-sep">. In: </span><span class="tei-series"
                    ><xsl:apply-templates mode="record-data"/></span>
        </xsl:if>
    </xsl:template>
    ...
    <xd:doc>
        <xd:desc>Count div elemetents that are ancestor of this node and have a tei:head
            element</xd:desc>
    </xd:doc>
    <xsl:template name="tei-div-count">
        <xsl:variable name="div-count-raw"
            select="count(./ancestor-or-self::tei:div[tei:head | @type])"/>
        <xsl:choose>
    ...
    <xd:doc>
        <xd:desc>Utillity "function" to get the contetn into the right header tag <xd:p>To generate
                a contents (back) link set the variable $contents-link to the approproiate link
                target (#id). </xd:p>
        </xd:desc>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>tei:p elements are mapped to html:p elements unless they reside inside another
            tei:p element (e.g. in the construct tei:p/tei:figure/tei:p). <xd:b>This does not use
                the <xd:ref>classnames</xd:ref> template to produce CSS class names when a @rend
                attribute is present.</xd:b>
        </xd:desc>
    </xd:doc>
...
-->
    ...
    <xsl:template
        match="
            tei:ptr[not(contains(@target, '.JPG') or
            contains(@target, '.jpg') or
            contains(@target, '.PNG') or
            contains(@target, '.png') or
            substring(@target, 1, 1) = '#')]"
        mode="record-data">
    ...
    <xsl:template match="cell | tei:cell" mode="record-data">
        <td>
            ...
            </xsl:choose> <xsl:apply-templates mode="record-data"/>
        </td>
    </xsl:template>
    <!--    <xd:doc>
    ...
    <xsl:template match="entry | tei:entry" mode="record-data">
        <div class="profiletext">
            ...
            <xsl:apply-templates mode="record-data"/>
                        <xsl:if
                            test="not((ancestor::tei:TEI|ancestor::TEI)//*[local-name(.) = 'seg' and @type='whitespace'])">
    ...
    <xd:doc>
        <xd:desc>Lemma form <xd:p> Default priority="0.5" We do enforce latin before arabic script.
            </xd:p>
        </xd:desc>
    </xd:doc>
    <xsl:template match="tei:form[@type = 'lemma' or @type = 'multiWordUnit']" mode="record-data">
        <span class="tei-form-{@type}">
            <xsl:apply-templates
                select="tei:orth[contains(@xml:lang, '-vicav')] | tei:orth[@xml:lang = 'ar']"
                mode="record-data"/>
            <xsl:text> </xsl:text>
            <xsl:apply-templates
                select="tei:orth[not(contains(@xml:lang, '-vicav')) and (@xml:lang != 'ar')]"
                mode="record-data"/>
            <xsl:apply-templates select="tei:orth[not(@xml:lang)]" mode="record-data"/>
            <xsl:apply-templates select="*[not(name() = 'orth' or name() = 'bibl')]"
                mode="record-data"/>
        </span>
    </xsl:template>
    ...
    <xsl:template name="first-of-group">
        <xsl:if
            test="(@type != preceding-sibling::*[1]/@type) or (normalize-space(preceding-sibling::*[1]/text()) = '')"
            >xsl-first-of-group</xsl:if>
    </xsl:template>
        ...
        <div class="tei-sense">
            <xsl:if test="tei:form[@type = 'construction']">
                <div class="tei-form-construction">
                    <xsl:apply-templates select="tei:form[@type = 'construction']"
                        mode="record-data"/>
                </div>
            </xsl:if>
            <xsl:if test="tei:def">
                <div class="tei-defs">
                    ...
                    <xsl:apply-templates
                        select="tei:def[@xml:lang = 'de' or (@lang | @xml:lang) = 'deu']"
                        mode="record-data"/>
            ...
            <xsl:if test="tei:usg[@type = 'dom']">
                ...
                <xsl:if test="def[@xml:lang = 'de']">
                    <xsl:text xml:space="preserve"> </xsl:text>
                    <span style="color:rgb(126,126,126); font-style: italic">(<xsl:value-of
                            select="def[@xml:lang = 'de']"/>)</span>
                </xsl:if>
    ...
    <xsl:template name="_tei_entry">
        <xsl:for-each select="tei:sense">
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:apply-templates
                        select="../tei:form[@type = 'lemma'] | ../tei:form[@type = 'multiWordUnit']"
                        mode="record-data"/>
                    ...
                    <xsl:if test="not(@rend) or ./@rend != 'singular_only'">
                        <xsl:apply-templates select="../tei:form[@type = 'inflected']"
                            mode="record-data"/>
                    </xsl:if>
    ...
    <xsl:template match="head[parent::div] | tei:head[parent::tei:div | parent::tei:body]"
        mode="record-data">
        <div class="tei-head">
    ...
    <xd:doc>
        <xd:desc>headings other than list and division headings are transformed into a generic html
            span</xd:desc>
    </xd:doc>
    <xsl:template match="head[not(parent::div)] | tei:head[not(parent::tei:div | parent::tei:body)]"
        mode="record-data">
        <xsl:variable name="class">
            <xsl:call-template name="classnames"/>
        </xsl:variable>
    ...
    <xd:doc>
        <xd:desc>lb elements may also be used to mark up horizontal rules of various types. These
            are distinguished by values in the @rend attribute</xd:desc>
    </xd:doc>
    ...
    <xd:doc>
        <xd:desc>although they are block level elements, tei:lg are transformed into html:spans,
            because they may reside inside a tei:p (html:p) element, e.g. in tei:p/tei:figure/tei:lg
            - block level rendering has to be done with CSS styling.</xd:desc>
    </xd:doc>
    ...
    <xsl:template match="milestone | tei:milestone" mode="record-data">
        ...
        <xsl:choose>
            <xsl:when test="@type[. = 'hr'] and @rend[. = 'line']">
                <span class="{$class}">&#160;</span>
                <span style="color:red;">[usage of <code>milestone type="hr"</code> is deprecated,
                    use tei:lb instead]</span>
            </xsl:when>
            ...
            <xsl:when
                test="@type[. = 'separator'] and (@rend[. = 'undefined'] or @unit[. = 'undetermined'])">
                <p class="{$class}">⌫⌦</p>
            </xsl:when>
            <xsl:when test="@type[. = 'symbol'] and @rend[. = 'blEtc']">રc.</xsl:when>
            <xsl:when test="@type[. = 'symbol'] and @rend[. = 'brackets' or . = 'bracketsTC']">
                <xsl:if
                    test="preceding-sibling::*[1]/*[self::milestone or self::tei:milestone]/@rend[. = 'brackets' or . = 'bracketsTC']">
                    <xsl:text>&#160;</xsl:text>
                </xsl:if>
                <span class="{$class}" style="font-size:18pt;">)(</span>
                <span style="color:red;">[usage of <code>milestone type="symbol"
                        rend="brackets"</code> is deprecated, use g element with @ref='#brackets'
                    instead]</span>
            </xsl:when>
            <xsl:when test="@type[. = 'symbol'] and @rend[. = 'flower']">✾ <span style="color:red;"
                    >[usage of <code>milestone type="symbol" rend="flower"</code> is deprecated, use
                    appropriate unicode character instead]</span>
            </xsl:when>
            <xsl:when test="@type[. = 'symbol'] and @rend[. = 'footerDots']">
                <span class="{$class}">.&#160;<sup
                        style="font-size: inherit;position: relative;top: -5px;"
                    >.</sup>&#160;.</span>
                <span style="color:red;">[usage of <code>milestone type="symbol"
                        rend="footerDots"</code> is deprecated, use <code>&lt;g
                        ref=#footerdots&gt;</code> instead]</span>
            </xsl:when>
            <xsl:when test="@type[. = 'symbol'] and @rend[. = 'Aquarius']">
                <span style="font-size:14pt;">♒</span>
                <span style="color:red;">[usage of <code>milestone type="symbol"
                        rend="Aquarius"</code> is deprecated, use appropriate unicode character
                    instead]</span>
            </xsl:when>
            <xsl:when test="@type[. = 'symbol'] and @rend[. = 'Mars']">
                <span style="font-size:14pt;">♂</span>
                <span style="color:red;">[usage of <code>milestone type="symbol" rend="Mars"</code>
                    is deprecated, use appropriate unicode character instead]</span>
            </xsl:when>
            <xsl:when test="@type[. = 'symbol'] and @rend[. = 'Saturn']">
                <span style="font-size:14pt;">♄</span>
                <span style="color:red;">[usage of <code>milestone type="symbol"
                        rend="Saturn"</code> is deprecated, use appropriate unicode character
                    instead]</span>
            </xsl:when>
            <xsl:when test="@type[. = 'symbol'] and @rend[. = 'Jupiter']">
                <span style="font-size:14pt;">♃</span>
                <span style="color:red;">[usage of <code>milestone type="symbol"
                        rend="Jupiter"</code> is deprecated, use appropriate unicode character
                    instead]</span>
            </xsl:when>
            <xsl:when test="@type[. = 'symbol'] and @rend[. = 'Merkur']">
                <span style="font-size:14pt;">☿</span>
                <span style="color:red;">[usage of <code>milestone type="symbol"
                        rend="Merkur"</code> is deprecated, use appropriate unicode character
                    instead]</span>
            </xsl:when>
            <xsl:when test="@type[. = 'symbol'] and @rend[. = 'Virgo']">
                <span style="font-size:14pt;">♍</span>
                <span style="color:red;">[usage of <code>milestone type="symbol" rend="Virgo"</code>
                    is deprecated, use appropriate unicode character instead]</span>
            </xsl:when>
        ...
        <span class="{$class}">
            <xsl:choose>
                ...
                <xsl:when test="@ref = '#brackets'">
                    <xsl:if
                        test="preceding-sibling::*[1]/*[self::g or self::tei:g][@rend = 'brackets']">
                        <xsl:text>&#160;</xsl:text>
                    </xsl:if>
                    <span class="{$class}" style="font-size:18pt;">)(</span>
                </xsl:when>
    ...
    <xsl:template match="tei:w" mode="record-data">
        ...
        returns empty string and number(false) = 0, number(true) = 1. -->
                ...
                <xsl:variable name="pos"
                    select="
                        concat(substring('pos ', number(not(tei:fs/tei:f[@name = 'pos'])) * string-length('pos ') + 1),
