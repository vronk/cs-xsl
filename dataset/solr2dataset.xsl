...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:utils="http://aac.ac.at/corpus_shell/utils" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ds="http://aac.ac.at/corpus_shell/dataset" xmlns:exsl="http://exslt.org/common" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" version="2.0" extension-element-prefixes="exsl xs xd utils">
...
<!--    <xsl:param name="qxkey" select="utils:params('qxkey',())" />-->
        ...
        <!-- try to match qkeys based on their position - hoping for parallel ordering of qx and qxkey -->
                    ...
                    <xsl:variable name="qkey" select="$qkey-list/*[not(.='')][position()=$curr_pos]"/>
                    ...
                    <xsl:choose>
                        <xsl:when test="number($count)=number($count)"> <!-- test if number -->
                            <xsl:attribute name="formatted">
                                <xsl:value-of select="utils:format-number($count,$number-format-default)"/>
                            </xsl:attribute>
          ...
          <!-- take first result (of first stat.field) as base for the labels -->
            ...
            <!-- and a dataseries for every response -->
              ...
              <!-- and a dataseries for every metrics in the response (min, max, sum...) -->
                    <xsl:variable name="metrics" select="distinct-values(lst[@name = 'stats']/*/lst[1]/lst[@name = 'facets']/lst[@name=$curr_facet]/lst/(double|long)/@name)[.=$statsx_metrics or $statsx_metrics='all'] "/>
...
<!--        <xsl:when test="$reldata=1"> <!-\- not reliable!! testing-\-> -->
    ...
    <xsl:template match="ds:value" mode="reldata">
        ...
        <ds:value label="{@label}" formatted="{@formatted}" abs="{.}" rel="{$relfreq}" rel_formatted="{utils:format-number($relfreq * $percentile-base, $number-format-dec)}">
