...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:cmd="http://www.clarin.eu/cmd/" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" version="2.0">
    ...
    <xsl:template name="continue-root">
        <xsl:apply-templates select="/cmd:CMD/cmd:Header" mode="record-data">
            <xsl:with-param name="strict" select="true()"/>
        </xsl:apply-templates>
        <xsl:apply-templates select="/cmd:CMD/cmd:Resources/cmd:ResourceProxyList" mode="record-data">
            <xsl:with-param name="strict" select="true()"/>
        </xsl:apply-templates>
        <xsl:apply-templates select="/cmd:CMD/cmd:Resources/cmd:IsPartOfList" mode="record-data">
            <xsl:with-param name="strict" select="true()"/>
        </xsl:apply-templates>
        <xsl:apply-templates select="/cmd:CMD/cmd:Components/*" mode="record-data">
            <xsl:with-param name="strict" select="true()"/>
        </xsl:apply-templates>
    </xsl:template>
</xsl:stylesheet>